...
module Breakout
  ...
  def self.grid_access_token(outlet, bid, e, notify)
    (Digest::SHA2.new << "#{CONFIG[:grid_key]}#{outlet}#{bid}#{e}#{notify}").to_s
  end
  ...
  def self.browser_url(outlet, opts={})
    bid = opts[:bid] || random_bid
    e = opts[:e] || (Time.now + 3).to_i
    notify = opts[:notify] || false
    gat = grid_access_token(outlet, bid, e, notify)
    "ws://#{CONFIG[:breakout_host]}:#{CONFIG[:browser_port]}/#{CONFIG[:grid]}?outlet=#{outlet}&bid=#{bid}&e=#{e}&notify=#{notify}&gat=#{gat}"
  end
